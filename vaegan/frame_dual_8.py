...
#       label_small = tf.small(label_big, pool_size=[4,1], strides=[4,1], padding='same',name="subsizing")
	...
	def build_model(self):
		...
		text_label_embedding = tf.placeholder(tf.float32, shape=[self.batch_size, self.word_len, 300])
		...
		for i in range(self.frames):
			...
			list_return = self.create_frames(next_image_input, x[:,:,:,3*i:3*i+3], 
				z_s[self.batch_size*i:self.batch_size*i+self.batch_size], z_c[self.batch_size*i:self.batch_size*i+self.batch_size], 
				image_class_input, text_label_input, z_t[self.batch_size*i:self.batch_size*i+self.batch_size])
			list_gen.append(list_return[0])
			next_image_input = tf.concat(axis=3, values=[next_image_input[:,:,:,3:], list_gen[-1]])
		x_generated = tf.concat(axis=3, values=list_values[0])
		print("Completed generation creation")
		...
		return placeholders, optimizer, losses, x_generated
...
gan = VAEGAN(batch_size=batch_size, embedding_size=embedding_size, image_shape=[64,64,3], 
	num_class_motion=num_class_motion, num_class_image=num_class_image, frames=frames, video_create=False, 
	word_len=word_len, frames_input = 3)
