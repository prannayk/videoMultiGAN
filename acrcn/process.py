...
for folder in folders : 
	...
	for file in filelist:
		...
		os.system("ffmpeg -i %s/%s.avi -vf fps=20 -s 40x32 -f image2 %s/%s/%s-"%(path, filename, path, filename, filename) + "%03d.png ")
		...
		for i,img in enumerate(images) : 
			...
			print(im)
			frames[i] = im
		np.save("%s/video_10_%s.npy"%(direc, filename), frames)
		os.system("rm -rf %s/%s")
