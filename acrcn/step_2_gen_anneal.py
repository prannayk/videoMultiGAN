...
class VAEGAN():
	...
	def build_model(self):
		lambda_var = tf.Variable(tf.random_normal(shape=[1], stddev=0.1))
		set_lambda = tf.assign(lambda_var, self.lambda_1, name="lambda_var_assignment")
		anneal = tf.assign(lambda_var, set_lambda_var*(1-self.eta_1))
		...
		with tf.variable_scope("losses"):
			...
			losses["generator_image"] = self.gan_loss(D_x_gen, True) + (lambda_var*losses["reconstruction"])  + self.gan_loss(D_x_hat, True) + self.gan_loss(D_x_dash, True) 
			losses["generator_image_gan"] = self.gan_loss(D_x_gen, True) + (lambda_var*losses["reconstruction"])  + self.gan_loss(D_x_hat, True) + self.gan_loss(D_x_dash, True) 
			losses["text_encoder"] = losses["gen_text_classifier"] + (losses["reconstruction"]*lambda_var)
			...
			losses["encoder"] = losses["gen_image_classifier"] + (lambda_var*losses["reconstruction"]) + losses["gen_style_classifier"]
		...
		with tf.variable_scope("optimizers"):
			...
			optimizer["generator_reconstruction"] = tf.train.AdamOptimizer(self.learning_rate[0],beta1=0.5,beta2=0.9).minimize(lambda_var*losses["reconstruction"], var_list=variable_dict["generator"])
			print("disc_image")
