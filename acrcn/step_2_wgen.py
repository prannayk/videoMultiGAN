...
class VAEGAN():
	...
	def build_model(self):
		...
		G_z_t_loss = tf.reduce_mean(tf.stack(axis=0, values=list_values[7]))
		D_z_c_loss = tf.reduce_mean(tf.stack(axis=0, values=list_values[5]))
		G_z_c_loss = tf.reduce_mean(tf.stack(axis=0, values=list_values[6]))
