...
class DCGAN():
	def __init__ (self, batch_size, image_shape, embedding_size, 
		...
		device = '/gpu:0', learning_rate_1 = 1e-3, learning_rate_2 = 1e-3):
...
#			g_cost = g_image
			...
			with tf.variable_scope('generator') as scope:
				...
				optimizer_gen = tf.train.AdamOptimizer(self.learning_rate_1,beta1=0.5).minimize(self.losses['gen'], 
					var_list=variables)
			with tf.variable_scope('discriminator') as scope:
				variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="discriminator")
				optimizer_disc = tf.train.AdamOptimizer(self.learning_rate_2,beta1=0.5).minimize(self.losses['disc'],
					var_list=variables)
			self.optimizers = {
				'gen' : optimizer_gen,
				'disc' : optimizer_disc
...
gan = DCGAN(batch_size, [32, 32,1 ], embedding_size, num_class=10, 
	learning_rate_1=lr1, learning_rate_2=lr2)
gan.build_model()
gan.start()
samples = generator()
