...
class DCGAN():
	def __init__ (self, batch_size = 50, image_shape = [28,28,1], embedding_size = 128, num_class =10, dim1 = 1024, dim2 = 128, dim3 = 64, frames=8, dim_channel = 1, dim4=16, learning_rate_1=sys.argv[1], learning_rate_2=sys.argv[2]):
...
for ep in range(epoch):
	...
	for t in range(64000 // batch_size):
		...
		if t%10 == 0 and t>0:
			print("Batches " + str(t*batch_size) + "done. " + str(g_loss_val) + "+" + str(d_loss_val) + " = " + str(d_loss_val + g_loss_val) + " in " + str(time.time()-start_time))
			start_time = time.time()
